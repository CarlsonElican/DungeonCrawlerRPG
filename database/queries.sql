...
WHERE NOT EXISTS (SELECT 1 FROM event_templates WHERE name = 'Robbed While Sleeping');
...
WHERE NOT EXISTS (SELECT 1 FROM event_results WHERE result_type = 'Robbed While Sleeping');

DELETE FROM event_template_results
WHERE event_template_id IN (
    SELECT event_template_id FROM event_templates WHERE name IN ('Loot Corpse', 'Hidden Treasure Room', 'Robbed While Sleeping')
);
...
INSERT INTO event_template_results (event_template_id, event_result_id, weight, min_floor, max_floor)
SELECT et.event_template_id, er.event_result_id, 1.00, 1, NULL
FROM event_templates et JOIN event_results er ON er.result_type = 'Robbed While Sleeping' WHERE et.name = 'Robbed While Sleeping';
